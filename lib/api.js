...
export function getZoneData(zoneId) {
    ...
    if (!summary || !summary.current || !summary.current.zones || !zoneId) return null;
    const zoneStr = zoneId.toString();
    const formattedZoneId = zoneStr.toLowerCase().startsWith('zone') ? zoneStr : `Zone ${zoneStr}`;
...
export const getClubDetails = cache(function getClubDetails(clubId) {
