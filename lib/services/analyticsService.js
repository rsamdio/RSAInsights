...
export async function getLeaderboards({
    ...
    if (['districts_by_growth', 'districts_growth', 'growth'].includes(cleanCategory)) {
 ...
 * @returns {Object|null}
 */
export async function getWorldwideStats({ type = 'all', country = '', limit } = {}) {
    ...
    if (type === 'country') {
        ...
        if (targetCountry) {
            countries = countries.filter(c => (c.country || c['Country/Geographic Area'] || '').toLowerCase().includes(targetCountry));
        }
    ...
    if (type === 'district') {
        ...
        return { total: districts.length, districts };
    }
    ...
    if (type === 'interact') {
        return {
            ...
            totalActiveInteractClubs: raw.totalActiveInteractClubs,
            totalSuspendedInteractClubs: raw.totalSuspendedInteractClubs,
            districts: safeLimit ? (raw.interactDistrictData || []).slice(0, safeLimit) : raw.interactDistrictData,
    ...
    if (type === 'new_clubs') {
        return {
            totalNewClubs: raw.totalNewClubs,
            countries: safeLimit ? (raw.newClubsCountryData || []).slice(0, safeLimit) : raw.newClubsCountryData,
            districts: safeLimit ? (raw.newClubsDistrictData || []).slice(0, safeLimit) : raw.newClubsDistrictData
        };
    }
