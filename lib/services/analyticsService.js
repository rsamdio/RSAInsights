...
export async function searchClubs({
    ...
    let filtered = allClubs.filter(c => {
        ...
        if (typeof effectiveIsArrears === 'boolean') {
            const clubIsArrears = Boolean(c.isArrears || c.Arrears === 'Yes');
            if (clubIsArrears !== effectiveIsArrears) return false;
        }
        if (typeof effectiveIsAtRisk === 'boolean') {
            const clubIsAtRisk = Boolean(c.isAtRisk);
            if (clubIsAtRisk !== effectiveIsAtRisk) return false;
        }
        if (typeof effectiveIsNoOfficers === 'boolean') {
            const clubIsNoOfficers = Boolean(c.isNoOfficers || c.Officers === 'No');
            if (clubIsNoOfficers !== effectiveIsNoOfficers) return false;
        }
        if (queryLower) {
