...
export function createSseResponse(request, endpointPath = '/api/mcp') {
    ...
    const targetUrl = new URL(endpointPath, dummyBase);
    targetUrl.searchParams.set('sessionId', sessionId);
    const relativeEndpointWithSession = targetUrl.pathname + targetUrl.search;
    ...
    const stream = new ReadableStream({
        start(controller) {
            ...
            controller.enqueue(encoder.encode(`event: endpoint\ndata: ${relativeEndpointWithSession}\n\n`));
            ...
            // Heartbeat to keep intermediate proxies (Cloudflare, Netlify) from dropping connection
