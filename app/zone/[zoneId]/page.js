import { getZoneData, getZoneSummary, getArrears, getNoOfficers, getRotaryNoSponsor, getDashboardSummary } from '@/lib/api';
...
export default async function ZonePage({ params }) {
    ...
    const prevZoneStats = prevSummary?.previous?.zones[fullZoneName]?.stats || null;

    const getDelta = (key, format = 'number') => {
        if (!prevZoneStats || (!prevZoneStats[key] && prevZoneStats[key] !== 0)) return null;
        const diff = stats[key] - prevZoneStats[key];
        if (diff === 0) return null;
        
        const pct = prevZoneStats[key] ? Math.abs((diff / prevZoneStats[key]) * 100).toFixed(1) : 0;
